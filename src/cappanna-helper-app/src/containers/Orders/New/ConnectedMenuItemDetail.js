...
class ConnectedMenuItemDetail extends Component {
    render() {
        return <MenuItemDetail detail={this.props.detail} incrementOrderDetailQuantity={this.props.incrementOrderDetailQuantity} />;
    }

    shouldComponentUpdate(nextProps) {
        if (this.props.detail.quantity !== nextProps.detail.quantity || this.props.detail.item.unitsInStock !== nextProps.detail.item.unitsInStock) {
            return true;
        }
...
const mapDispatchToProps = (dispatch) => {
    return {
        incrementOrderDetailQuantity: (itemId, quantity, price) => dispatch(incrementOrderDetailQuantity(itemId, quantity, price))
    };
};
