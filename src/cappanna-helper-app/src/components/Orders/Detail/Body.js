...
import { loadSelectedOrderRequested } from "actions";
...
const Body = () => {
  ...
  const dishList = order ? buildFilledOrderDetails(order.details, menuDetails) : [];
  ...
  return (
    <Root>
      ...
      <TextField variant="standard" label="Note" multiline readOnly fullWidth value={notes} className={classes.notes} />
    </Root>
  );
};
