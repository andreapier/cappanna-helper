...
function* orderChanged(action) {
  ...
  if (
    state.newOrderHeader.id === action.payload.id &&
    lastOperation.userId !== state.user.userId
  ) {
    if (action.type === ORDER_DELETED) {
      yield put(
        notifyWarning("L'ordine è stato cancellato, ricrearlo se necessario")
      );
      history.push("order/new");
    } else if (action.type === ORDER_CLOSED) {
      yield put(
        notifyWarning(
          "L'ordine è stato chiuso, non è più possibile modificarlo"
        )
      );
...
function* orderChangedSaga() {
  yield takeLatest(
    [ORDER_CHANGED, ORDER_PRINTED, ORDER_DELETED, ORDER_CLOSED],
    orderChanged
  );
}
